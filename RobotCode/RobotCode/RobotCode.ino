...
void loop()
{
	...
	else if ((distance_left > (minimum_left + 5)) && (distance_right > (minimum_right + 5)) && (distance_front > (minimum_front + 10))) {
	...
	//}
		...
		// if both left && right are clear 
		else if ((distance_right > minimum_right) && (distance_left > minimum_left))
		{
			last_turn = EEPROM.read(2);
			Serial.print(last_turn);

			if (last_turn == 0) // hasnt turned left || right already at T junction 
			{

				Serial.print("hasnt turned left || righttttt");
			...
			else if (last_turn == 2) // if it's turned both left && right, calculate quickest route 
			{

				Serial.print("turned already left && right");
				...
				{
					Serial.print("##Turning RIGHT because already turned both ways && right is quicker!!! ##");
					// Attach servos again
					servoLeft.attach(2);
					servoRight.attach(3);
					// turn right 
				...
				{ // turn left 
					Serial.print("##Turning LEFT because already turned both ways && LEFT is quicker!!! ##");
					// Attach servos again
					servoLeft.attach(2);
					servoRight.attach(3);
					// turn left  
		...
		else if ((distance_right < minimum_right) && (distance_left < minimum_left) && (distance_front < minimum_front))
		{
...
{
	if (abs(distance_left - distance_right) > 3 && abs(distance_left - distance_right) < 15)
	{
...
void adjust_func() {
	...
	if (abs(distance_left - distance_right) > 3 && abs(distance_left - distance_right) < 8) {
		//sensorRefresh();
		...
		if ((distance_left > distance_right) && (adjust == true)) {
		...
		else if ((distance_left < distance_right) && (adjust == true)) {
			...
			servoLeft.detach();
			servoRight.detach();
			Serial.print("Detatched, && adjusting");
			servoLeft.attach(2);
			servoRight.attach(3);
			servoLeft.writeMicroseconds(1600);         // Left wheel counterclockwise
			servoRight.writeMicroseconds(1300);
